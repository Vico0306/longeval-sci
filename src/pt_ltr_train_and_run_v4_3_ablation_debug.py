...
def load_fixed_split(path: Path):
    if not path.exists():
        raise FileNotFoundError(f"Split-Datei fehlt: {path}")
...
# Feature Engineering
# -------------------------
def add_features(df: pd.DataFrame, qid_to_snap: dict, include_time: bool) -> pd.DataFrame:
    out = df.copy()
...
def ndcg_at_k_from_lists(rels, k=10):
    # rels is list in rank order
    import math
    def dcg(rs):
        s = 0.0
        for i, r in enumerate(rs[:k], start=1):
...
def per_query_ndcg(run_df: pd.DataFrame, qrels_df: pd.DataFrame, score_col: str, k=10) -> pd.DataFrame:
    # run_df columns: qid, docno, score_col
    # qrels_df: qid, docno, label
    merged = run_df.merge(qrels_df, on=["qid", "docno"], how="left").fillna({"label": 0})
    merged = merged.sort_values(["qid", score_col], ascending=[True, False]).copy()
    merged["rank"] = merged.groupby("qid").cumcount()
    merged = merged[merged["rank"] < k]

    rows = []
    for qid, g in merged.groupby("qid", sort=False):
        rels = g["label"].astype(int).tolist()
        rows.append({"qid": qid, "ndcg": ndcg_at_k_from_lists(rels, k=k)})
    return pd.DataFrame(rows)
...
def main():
    ...
    ap.add_argument("--no_time_features", action="store_true", help="Train LTR without snapshot/age time features")
    ap.add_argument("--k", type=int, default=10)
    args = ap.parse_args()

    include_time = not args.no_time_features
    ...
    test_l = cand_test.copy()

    train_l = add_features(train_l, qid_to_snap, include_time)
    val_l = add_features(val_l, qid_to_snap, include_time)
    test_l = add_features(test_l, qid_to_snap, include_time)

    # feature set
    base_features = ["f_bm25", "f_qlen", "f_doclen", "f_title_len", "f_abs_len", "f_year"]
    time_features = ["f_snap_year", "f_snap_month", "f_age_years", "f_is_recent_24m"]
    feature_cols = base_features + (time_features if include_time else [])
    ...
    ltr_out = RUNS_DIR / ("pt_ltr_test_notime.jsonl" if not include_time else "pt_ltr_test.jsonl")
    ...
    # debug compare per query
    qrels_small = qrels.copy()
    bm25_nd = per_query_ndcg(test_l, qrels_small, score_col="score", k=args.k).rename(columns={"ndcg": "bm25_ndcg"})
    ltr_nd = per_query_ndcg(test_l, qrels_small, score_col="ltr_score", k=args.k).rename(columns={"ndcg": "ltr_ndcg"})
    comp = bm25_nd.merge(ltr_nd, on="qid", how="inner")
    comp["diff"] = comp["ltr_ndcg"] - comp["bm25_ndcg"]
    comp.to_csv(RUNS_DIR / "pt_compare_test.csv", index=False)

    better = int((comp["diff"] > 0.0001).sum())
    worse = int((comp["diff"] < -0.0001).sum())
    same = len(comp) - better - worse
    print(f"COMPARE: better={better} | worse={worse} | same={same} | mean_diff={comp['diff'].mean():.4f}")
    print("WROTE:", (RUNS_DIR / "pt_compare_test.csv").resolve())
    print("DONE.")

...
DEFAULT_MODEL_PATH = MODEL_DIR / "ltr_v1_alpha025.joblib"
...
def main():
    ...
    ap.add_argument("--model_out", type=str, default=str(DEFAULT_MODEL_PATH))
    ...
    train_qids, val_qids, test_qids = fixed_split(all_qids, seed=args.seed)

    print(f"[TRAIN] SPLIT: train={len(train_qids)} | val={len(val_qids)} | test={len(test_qids)}")
    ...
    print("[TRAIN] BM25 candidates train...")
    cand_train = retr.transform(q_train)
    print("[TRAIN] BM25 candidates val...")
    cand_val = retr.transform(q_val)
    print("[TRAIN] BM25 candidates test...")
    cand_test = retr.transform(q_test)

    print(f"[TRAIN] cand_train rows={len(cand_train)} | qids={cand_train['qid'].nunique()}")
    print(f"[TRAIN] cand_val   rows={len(cand_val)}   | qids={cand_val['qid'].nunique()}")
    print(f"[TRAIN] cand_test  rows={len(cand_test)}  | qids={cand_test['qid'].nunique()}")
    ...
    cand_train = add_dense_feature(
        cand_train, q_train, model_name=args.dense_model, batch_size=args.batch_size, verbose=True
    )
    cand_val = add_dense_feature(
        cand_val, q_val, model_name=args.dense_model, batch_size=args.batch_size, verbose=True
    )

    for df in (cand_train, cand_val):
    ...
    print(f"[TRAIN] TRAIN rows={len(train)} | groups={len(train_group)} | positives={(train['rel'] > 0).sum()}")
    print(f"[TRAIN] VAL   rows={len(val)}   | groups={len(val_group)}   | positives={(val['rel'] > 0).sum()}")
    ...
    if args.monotone_bm25:
        params["monotone_constraints"] = [1 if c == "f_bm25" else 0 for c in feature_cols]

    ranker = lgb.LGBMRanker(**params)

    print("[TRAIN] Training with early stopping...")
    ranker.fit(
        X_train,
        y_train,
        group=train_group,
    ...
    print("[TRAIN] best_iteration =", ranker.best_iteration_)
    ...
    print(f"[TRAIN] Model saved to: {model_out.resolve()}")

...
def candidate_recall_at_k(cands: pd.DataFrame, qrels: pd.DataFrame, k: int) -> float:
    """
    Recall@k über qrels: Anteil der (qid,docno) mit rel>0, die in den Top-k Candidates vorkommen.
    """
...
def main():
    ...
    # Debug / safety
    ap.add_argument("--overfit_debug", action="store_true", help="Train=Val=Test auf denselben Queries (Sanity Check)")
    ap.add_argument("--alpha", type=float, default=0.0, help="final_score = bm25 + alpha*ltr (0.0 = nur ltr_score)")
    ap.add_argument("--recall_k", type=int, default=1000, help="Candidate recall@K messen")
    ap.add_argument("--monotone_bm25", action="store_true", help="Monotone constraint: f_bm25 muss positiv wirken")
    ...
    if args.overfit_debug:
        # alle qids in allen splits
        train_qids = all_qids
        val_qids = all_qids
        test_qids = all_qids

    print(f"SPLIT: train={len(train_qids)} | val={len(val_qids)} | test={len(test_qids)} | overfit={args.overfit_debug}")
    ...
    # Recall sanity (nur wenn nicht overfit debug, sonst ist es riesig)
    try:
        r_train = candidate_recall_at_k(cand_train, qrels, min(args.recall_k, args.topk))
        r_test = candidate_recall_at_k(cand_test, qrels, min(args.recall_k, args.topk))
        print(f"RECALL@{min(args.recall_k,args.topk)}: train={r_train:.4f} | test={r_test:.4f}")
    except Exception as e:
        print("RECALL compute failed:", e)
    ...
    # ---- LABELS ----
    train = attach_labels(cand_train, qrels)
    val = attach_labels(cand_val, qrels)
    test = attach_labels(cand_test, qrels)
    ...
    if args.monotone_bm25:
        # f_bm25 muss monoton positiv wirken
        # Reihenfolge muss zu feature_cols passen:
        # ["f_bm25", "f_dense", "f_qlen", "f_doclen", "f_title_len", "f_abs_len", "f_year"]
        params["monotone_constraints"] = [1, 0, 0, 0, 0, 0, 0]
    ...
    if args.alpha != 0.0:
        ...
        score_for_run = "final_score"
        print(f"USING final_score = bm25 + {args.alpha}*ltr")
    else:
        score_for_run = "ltr_score"
        print("USING ltr_score only")
    ...
    # LTR run
    ltr_out = test.sort_values(["qid", score_for_run], ascending=[True, False]).copy()
    ltr_out["rank"] = ltr_out.groupby("qid").cumcount()
    write_run_jsonl(ltr_out[["qid", "docno", "rank", score_for_run]], RUN_LTR_TEST, score_col=score_for_run)
